...
(function () {
  ...
  const ATMOSPHERE_KEYWORDS = /喵|蹭蹭|蹭|贴贴|Hug|摸摸|Pat|亲亲|ฅ|🐾|💗|💕|💖/i;
  ...
  const defaults = {
    ...
    actions: [
      {
        label: "Hug",
        text: "Gently hugs {target} and nuzzles with cheek meow~",
        selfText: "Hugs self and softly nuzzles own tail meow~",
      },
      {
        label: "Pat",
        text: "Tiptoes to pat {target}'s head and earnestly praises: Good kitty meow~",
        selfText: "Pats own head, pretending to be happy from praise meow~",
      },
      {
        label: "Feed",
        text: "Brings a small treat to {target}'s lips, wagging tail expectantly: Ahm meow~",
        selfText: "Holds small treat and nibbles it, squinting eyes contentedly meow~",
      },
    ],
  };
  ...
  function normalizeConfig(next) {
    ...
    next.actions = (Array.isArray(next.actions) && next.actions.length ? next.actions : fallbackActions)
      .map((action, index) => ({
        label: String(action.label || fallbackActions[index]?.label || "Action").slice(0, 6),
        text: String(action.text || fallbackActions[index]?.text || "Got a bit closer to {target} meow~"),
        selfText: String(action.selfText || fallbackActions[index]?.selfText || "Gently wags tail meow~"),
      }))
      .slice(0, 6);
    return next;
  }
    ...
    const normalized = actions
      .map((action, index) => {
        ...
        return {
          ...
          label: String(action.label || action.id || "Action").trim().slice(0, 6),
  ...
  function loadRemoteActionLibrary() {
    return requestText(ACTION_LIBRARY_URL)
      .then((text) => {
        ...
        renderWheel();
        console.log(`[BC Neko Enhancer] Action library loaded: ${library.version}, ${library.actions.length} themes`);
        return library;
      })
      .catch((error) => {
        console.warn("[BC Neko Enhancer] Remote action library failed, using cache/built-in:", error);
    ...
    const normalized = groups
      .map((group, index) => {
        ...
        return {
          ...
          label: String(group.label || group.id || "Kaomoji").trim().slice(0, 12),
  ...
  function loadRemoteKaomojiLibrary() {
    return requestText(KAOMOJI_LIBRARY_URL)
      ...
      .catch((error) => {
        console.warn("[BC Neko Enhancer] Remote kaomoji library failed, using cache/built-in:", error);
  ...
  function registerModSdk() {
    return loadModSdk()
      ...
      .catch((error) => {
        console.warn("[BC Neko Enhancer] BC Mod SDK failed to load, retrying later:", error);
        return null;
      });
  }
  ...
  function syncBodyState() {
    ...
    if (mainButton) {
      mainButton.title = config.menuCollapsed
        ? "Expand cat menu，Hold to drag，Hold 10s to toggle catgirl mode"
        : "Collapse cat menu，Hold to drag，Hold 10s to toggle catgirl mode";
    }
    const handleButton = document.getElementById("bcn-wheel-handle");
    if (handleButton) {
      handleButton.textContent = config.wheelCollapsed ? "🐱" : "🐱";
      handleButton.title = config.wheelCollapsed ? "Expand action wheel" : "Collapse action wheel";
    }
  }
  ...
  function whisperNeko(text) {
    ...
    return text.startsWith("Psst meow~") ? text : `Psst meow~ ${text}`;
  }
  ...
  function patchBC() {
    ...
    bcModApi.hookFunction("ChatRoomMessageDisplay", 0, (args, next) => {
      ...
      if (config.notifyIncoming && data?.Sender && !isOwnSender(data.Sender) && ["Chat", "Whisper"].includes(data.Type)) {
        showToast(data.Type === "Whisper" ? "Psst meow~ Private message incoming!" : "Meow~ New message arrived!");
      }
      return div;
    });
    ...
    console.log("[BC Neko Enhancer] Hooked chat functions via BC Mod SDK meow~");
    return true;
  }
  ...
  function drawNekoBadgeTooltip() {
    ...
    for (const [memberNumber, hitbox] of badgeHitboxes) {
      ...
      const label = isSelf ? `Catgirl Plugin v${version}` : `Fellow Catgirl v${version}`;
  ...
  function insertKaomoji(face) {
    const input = getChatInput();
    if (!input) {
      showToast("Chat box not found, click after entering chatroom meow~");
      return;
    }
    ...
    showToast("Catgirl kaomoji inserted meow~");
  }
  ...
  function toggleNekoMode(button) {
    ...
    if (toggleButton) {
      ...
      toggleButton.title = config.enabled ? "Disable catgirl mode" : "Enable catgirl mode";
    }
    showToast(config.enabled ? "Catgirl mode enabled meow~" : "Catgirl mode disabled");
  }

  function getCharacterName(character) {
    return W.CharacterNickname?.(character) || character?.Nickname || character?.Name || "target";
  }
  ...
  function showTargetPicker(action, anchor) {
    ...
    picker.innerHTML = `
      <div class="bcn-target-title">Select Target</div>
      <button type="button" data-self="1">Self</button>
  ...
  function editActions() {
    ...
    showToast("Action库现在从 GitHub JSON 管理喵~");
  }
  ...
  function createPanel() {
    ...
    panel.innerHTML = `
      ...
      <div id="bcn-kaomoji-picker" aria-label="Catgirl Kaomoji Picker"></div>
    `;
  ...
  const NekoSettingsUI = (() => {
    ...
    function drawBehaviorCard() {
      ...
      drawCardTitle(cards.middle.x + 62, cards.middle.y + 60, "⚙", "Behavior Settings");

      drawCheckBox(enabledRow.x, enabledRow.y, !!config.enabled);
      write("Catgirl Mode（enabled）", enabledRow.x + 70, enabledRow.y + 2, 24, theme.text, 700);
      write(config.enabled ? "Currently converting tone and enabling decorations" : "Currently paused, only settings entry remains", enabledRow.x + 70, enabledRow.y + 38, 18, theme.muted, 500);
  ...
  function registerSettingsUI() {
    ...
    console.log("[BC Neko Enhancer] Extension settings page registered");
    return true;
  }
